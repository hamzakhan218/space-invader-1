  ...
  for (let i = 0; i < armyRows; i++) {
    ...
    for(let j = 0; j < armyColumns; j++){
      ...
      armyArray[i][j] = {
        ...
        y: aY + i * (invaderHeight + invaderTopOffset),
        status: "alive",
        currentSpriteHeight: invaderSpriteHeightsArray[i][0], // Initialize with the first sprite height
      };
    }
  }
}
...
function drawArmyOfInvaders(){
  for (let i = 0; i < armyRows; i++) {
    for(let j = 0; j < armyColumns; j++){
        let soldier = armyArray[i][j];
        if (soldier.status=='alive') {
          drawInvader(soldier.x,soldier.y,soldier.currentSpriteHeight);
            if(soldier.y > tankY){
              gameRunning=false;
            }
        }
    ...
    for (let i = 0; i < armyRows; i++) {
      for(let j = 0; j < armyColumns; j++){
          ...
          if(
            ...
            tankBullet__y < soldier.y + invaderHeight && soldier.status == 'alive'
          )
          {
            console.log(tankBullet__x)
            console.log(tankBullet__y)
            console.log(i,j)
            soldier.status='dead';
            soldier.currentSpriteHeight = invaderSpriteHeightsArray[i][spriteSelector];
            shouldMoveTankBullet=false;
            aliveInvaders--;
            score+=5;            
            explosionColor = "red";
...
function drawScore(){
  drawBottomMessage("Score: "+score,canvas.width - 130)
}
function drawLives() {
  ctx.save();
  ctx.fillStyle = "red";
  ...
  for (let i = 0; i < lives; i++) {
    ctx.fillText("❤️", 20 + i * 30, canvas.height -10);
  }
